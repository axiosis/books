...
\newpage\subsection{Кодування Бома}
...
\begin{lstlisting}[mathescape=true]
module list where
  def map (A B: U) (f: A -> B) : list A -> list B
  def length (A: U): list A -> nat
  def append (A: U): list A -> list A -> list A
  def foldl (A B: U) (f: B -> A -> B) (Z: B): list A -> B
  def filter (A: U) (p: A -> bool) : list A -> list A
\end{lstlisting}
...
\newpage\subsection{Операційна семантика}
